...
/**
 * Java implementation of PTransE.
 *
...
public class PTransE {
    ...
    private static final File PATH2_FILE = new File(KGCompletion.KB2E_DIRECTORY, "path2.txt");
    private static final File CONFIDENCE_FILE = new File(KGCompletion.KB2E_DIRECTORY, "confidence.txt");
    ...
    private static void pathConstraintResourceAllocation() throws IOException {
        ...
        Log.info("PTransE.PCRA", "Loading training relations...");
        int trainingTriples = addTrainingRelations(pairRelationMap, headRelationTailMap,
                relationToId, relationCount);
        Log.info("PTransE.PCRA", String.format("Loaded %d training relations in %dms.",
                trainingTriples, System.currentTimeMillis() - startMs));

        startMs = System.currentTimeMillis();
        Log.info("PTransE.PCRA", "Loading eval pairs...");
        int evalPairs = addEvalPairs(pairRelationMap);
        Log.info("PTransE.PCRA", String.format("Loaded %d eval pairs in %dms.", evalPairs,
        ...
        startMs = System.currentTimeMillis();
        Log.info("PTransE.PCRA", "Executing Path-Constraint Resource Allocation...");
...
//                                        System.out.println("\t\t" + nestedPairKey + " " + nestedRelationKey + " " + delta);
                ...
                for (String tailId : headRelationTailMap.keySet()) {
                    ...
                    if (pathResources.containsKey(entityPairKey)) {
                        ...
                        path2Writer.write(reliablePathResources.size());
                ...
                Log.info("PTransE.PCRA", String.format("\tStep: %d of %d, totalPaths: %d, "
                        + "time: %dms", countedHeadEntities, headRelationTailMap.size(),
                        totalPaths, System.currentTimeMillis() - startMs));
            }
        }
        Log.info("PTransE.PCRA", String.format("Executed Path-Constraint Resource "
            ...
            for (String path : reliablePaths) {
                ...
                for (int i = 0; i < relationCount; i++) {
                    ...
                    if (pathCounts.containsKey(path) && relatedPathCounts.containsKey(pathKey)) {
                        float delta = relatedPathCounts.get(pathKey) / (float) pathCounts.get(path);
                        out.add(" " + i + " " + delta);
                    }
                }
                if (!out.isEmpty()) {
    ...
    private static void writePaths(Mode mode, Map<String, String> relationToId, int relationCount,
        ...
        try (BufferedReader br = FileTools.bufferedReader(mode.getTriplesFile());
            ...
            for (String line = br.readLine(); line != null; line = br.readLine()) {
                ...
                writePaths(bw, headId, tailId, relationId, pathResources);
                writePaths(bw, tailId, headId, getInverseRelationId(relationId, relationCount),
                        pathResources);
            }
        }
    }

    private static void writePaths(BufferedWriter bw, String headId, String tailId,
            String relationId, Map<String, Map<String, Float>> pathResources) throws IOException {
        bw.write(String.format("%s %s %s\n", headId, tailId, relationId));
